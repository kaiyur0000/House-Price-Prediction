...
def scattering():
    ...
    plt.title("Square Feet vs Price/Sqft")
    plt.show()
...
#-------------------------------------------------------------------------------------------------------------
        global reqrows
        reqrows=csvfilee[['BHK','total_sqft','price','site_location']][csvfilee['site_location']==siteloc][csvfilee['BHK']==bhk]
        
        length_bhk=len(reqrows['BHK'])        
        ...
        if(length_bhk > 10):
            ...
            drop_perfeet=reqrows[(reqrows.perfeet<lowerlimit)|(reqrows.perfeet>upperlimit)]


            # removing totalsqft outliers
            q1_sqft=reqrows.total_sqft.quantile(0.25)
            q3_sqft=reqrows.total_sqft.quantile(0.75)
            iqr=q3_sqft-q1_sqft
            lowerlimit1=q1_sqft-1.5*iqr
            upperlimit1=q3_sqft+1.5*iqr
            drop_total_sqft=reqrows[(reqrows.total_sqft<lowerlimit1)|(reqrows.total_sqft>upperlimit1)]
            # print(drop_total_sqft)
            # -----------------------------------------------------------------------------------------------------

            nooutlier=reqrows[(reqrows.total_sqft>lowerlimit1)&(reqrows.total_sqft<upperlimit1)][(reqrows.perfeet>lowerlimit)&(reqrows.perfeet<upperlimit)]
